...
SELECT title AS min_duration_title, length AS min_duration
FROM film
ORDER BY length ASC
LIMIT 1;

SELECT title AS max_duration_title, length AS max_duration
FROM film
...
SELECT ROUND(AVG(length)) AS average_duration
FROM film;
...
-- along with the first 3 characters of their email address, so that you can address them by their first name and use their email address to send personalized recommendations. The results should be ordered by last 
-- name in ascending order to make it easier to use the data.
...
SELECT 
	COUNT(film_id) as film_count, rating
FROM film
GROUP BY rating
...
SELECT 
	ROUND(AVG(length),2) as avg_duration, rating
FROM film
GROUP BY rating
